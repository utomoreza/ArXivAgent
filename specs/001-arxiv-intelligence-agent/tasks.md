...
- [ ] T011 Create initial Alembic revision `migrations/versions/0001_initial.py` — creates `date_status` and `chunk_type` enums first, then enables `vector` extension, then all tables and B-tree indexes; **does NOT create HNSW vector index** (created post-backfill); see `data-model.md` → Migration Notes for ordering rules
