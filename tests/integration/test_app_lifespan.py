...
async def test_lifespan_startup_and_shutdown():
    """Lifespan must initialise engine/factory, launch backfill in background, start scheduler, then tear down."""
    mock_engine = AsyncMock()
    mock_factory = MagicMock()
    mock_scheduler = MagicMock()

    # backfill runs as a background task — use an event to signal it started
    backfill_started = asyncio.Event()

    async def _fake_backfill(factory):
        backfill_started.set()
        await asyncio.sleep(0)  # yield so the task is observable
    ...
    with (
        ...
        patch("src.api.app.run_inception_backfill", side_effect=_fake_backfill),
        ...
        async with application.router.lifespan_context(application):
            ...
            assert backfill_started.is_set(), "backfill task was never started"
